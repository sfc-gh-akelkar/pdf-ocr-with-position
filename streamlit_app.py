...
def upload_pdf_with_progress(uploaded_file):
    ...
    st.markdown(f"""
    Your document **{uploaded_file.name}** is now ready! You can:
    
    - 🔍 **Start searching** by entering questions in the search box above
    - 📖 **Browse by page** using the tabs below  
    - 📚 **Select the document** from the sidebar to filter searches
...
try:
    ...
    if len(docs_df) > 0:
        ...
        uploaded_file = st.sidebar.file_uploader(
            "Upload PDF",
            type=['pdf'],
            help="Upload a PDF file (max 50MB) to make it searchable"
        )
            ...
            if not is_valid:
                ...
                try:
                    if not docs_df.empty and uploaded_file.name in docs_df['DOC_NAME'].values:
                        st.sidebar.warning("⚠️ Will replace existing file")
                except:
                    pass
